...
class frmContingencias (DialogType, DialogBase):
                ...
                if padre: #aparato
                    ...
                    if self.lblEvento.text()=="<Automatico>":
                        ...
                        try:
                            ...
                            cnn.commit()
                            QMessageBox.information(self, "EnerGis 6", "Borrado !")
                            self.listar(None)
                        except:
                            cnn.rollback()
    ...
    def registrar(self):
        ...
        for n in range (0, len(contingencias)):
            ...
            if tipo_elemento=="Elem_ManiobraMT":
                ...
                cursor.close()
                if len(nodo)==0:
                    QMessageBox.warning(self, "EnerGis 6", "❌ No se encontró el aparato " + nombre)
                else:
                    geoname = nodo[0][0]
                    aux = nodo[0][1]
                    cursor = self.conn.cursor()
                    ...
                    #except:
                    #    self.conn.rollback()
                    #    QMessageBox.warning(self, "EnerGis 6", "❌ No se pudo cargar la Operacón del Elemento ! id:" + str(id) + ", ult_ope:" + str(ult_ope) + ", valores:" + str_valores)
            if tipo_elemento=="Usuario":
                ...
                cursor.close()
                if len(usuarios)==0:
                    QMessageBox.warning(self, "EnerGis 6", "❌ No se encontró el usuario " + nombre)
                else:
                    usuario = nombre
    ...
    def afectados(self, nodo):
        ...
        try:
            ...
            QMessageBox.crtitical(None, "EnerGis 6", "❌ Error al calcular suministros afectados !")
